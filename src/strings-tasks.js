...
function isPalindrome(str) {
  const arrFromStr = str
    ...
    .replaceAll(',', '')
    .split('');
  const arrFromStrReversed = arrFromStr.slice().reverse();
  return arrFromStr.join('') === arrFromStrReversed.join('');
}
...
function findLongestWord(sentence) {
  ...
  let longestWord = '';
  for (let i = 0; i < arrFromsentence.length; i += 1) {
    if (arrFromsentence[i].length > longestWord.length) {
      longestWord = arrFromsentence[i];
    }
  }
  return longestWord;
}
...
function reverseWords(str) {
  ...
  const array2 = array1.map((el) => el.split(''));
  return array2.map((el) => el.reverse().join('')).join(' ');
}
...
function invertCase(str) {
  ...
  const result = [];
  for (let i = 0; i < str.length; i += 1) {
    if (alphabetLower.includes(str[i])) {
      result.push(str[i].toUpperCase());
    } else if (alphabetUpper.includes(str[i])) {
